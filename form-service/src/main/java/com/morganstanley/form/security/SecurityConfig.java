package com.morganstanley.form.security;
...
public class SecurityConfig {
    ...
    public UserDetailsService userDetailsService() {
        ...
        UserDetails deepa = User.builder()
            .username("deepa.ganesh@morganstanley.com")
            .password(passwordEncoder().encode("demo123"))
            .roles("USER", "ADMIN")
            .build();

        log.info("Created demo users: user (USER), admin (ADMIN), deepa.ganesh (ADMIN)");

        return new InMemoryUserDetailsManager(user, admin, deepa);
    }
